...
class Pose():
	...
	DropBall 		= [2000, -1250, 0]	# [2000, -1250, 0]
	KickOffHome 	= [750, -750, -45]	# [750, -750, -1045]
	KickOffAway 	= [500, -2250, 0]	# [500, -2250, 1000]
	GoalKickHome 	= [2000, -4750, 0]	# [2000, -4500, 0]
	GoalKickAway 	= [2000, -750, 0]	# [2000, -750, 0]
	FreeKickHome	= [2000, -1000, 0]	# [2000, -1000, 0]
	FreeKickAway	= [2000, -3250, 0]	# [2000, -3250, 0]
	CornerHome 		= [3500, 6250, 135]	# [3500, 6250, 1135]
	CornerAway 		= [3500, -2750, -180]	# [3500, -2750, -180]
	PenaltyHome 	= [0, -1000, 0]		# [0, -1000, 0]
	PenaltyAway 	= [3000, -3750, 0]		# [3000, -3750, 0]
	Defense			= [1000, -3750, 0]	# [1000, -3750, 0]
...
# rosbridge friend
jersey1_pose_pub = rospy.Publisher('/local_strategy/jersey1/pose', Pose2D, queue_size=10)
jersey1_state_pub = rospy.Publisher('/local_strategy/jersey1/state', Byte, queue_size=10)
# serial node
...
def Jersey2PoseHandler(data):
	global jersey2_pose
	jersey2_pose = data
	# rospy.loginfo(str(jersey2_pose.x) + "  " + str(jersey2_pose.y) + "  " + str(jersey2_pose.theta))

def Jersey2StateHandler(data):
	global jersey2_state
	jersey2_state = RobotState(data.data)
...
def ActionStatsHandler(data):
	...
	jersey1_state_pub.publish(data.data)
	# rospy.loginfo(RobotState(data.data))
...
def WaitUntilFriendReady(time_out=30):
	...
	WaitFriend()
	time_count = 0
	while jersey2_state != RobotState.WaitBall and enable != Enable.Stop:
		if rospy.is_shutdown():
			break
		if time_count >= time_out:
			break
	...
	sleep(0.25)
	SetAbsPose([robot_pose.x, robot_pose.y, GetAngle([jersey2_pose.x, jersey2_pose.y])])
...
def Play():
	global enable, arah_penalty
	while enable != Enable.Stop and enable != Enable.Empty:
		if jersey2_state != RobotState.GotoBall and jersey2_state != RobotState.SetDribblePose:
			GotoBall()
			if ball_lost:
				SetAbsPose([3000, -4000, 1000], ball_found_monitor=True)	# [3000, -4000, 1000]
				SetAbsPose([-2000, -2000, 1000], ball_found_monitor=True)	# [-3000, -2000, 1000]
				SetAbsPose([3000, 0, 1000], ball_found_monitor=True)		# [3000, 0, 1000]
				SetAbsPose([-2000, 2000, 1000], ball_found_monitor=True)	# [-3000, 2000, 1000]
				SetAbsPose([3000, 4000, 1000], ball_found_monitor=True)		# [3000, 4000, 1000]
			else:
...
if __name__ == '__main__':
	...
	# rosbridge friend
	rospy.Subscriber('/local_strategy/jersey2/pose', Pose2D, Jersey2PoseHandler)
	rospy.Subscriber('/local_strategy/jersey2/state', Byte, Jersey2StateHandler)
	# serial node
...
# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
	...
	while not rospy.is_shutdown():
		if enable == Enable.Stop:
			...
			if (front_ball_detected or omni_ball_detected) and jersey2_state != RobotState.Distance2m:
				Distance2m()
				here = True
			else:
